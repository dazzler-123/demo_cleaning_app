...
export const leadsRepository: LeadsRepository = {
  ...
  async findConfirmedWithScheduleAndAssignment(filters) {
    ...
    const leadIds = items.map((l) => l._id.toString());
    ...
    for (const s of schedules) {
      const id = s.leadId.toString();
      if (!schedulesMap.has(id)) schedulesMap.set(id, s);
    }

    const scheduleIds = [...schedulesMap.values()].map((s) => s._id.toString());
    ...
    for (const a of assignments) {
      assignmentsMap.set(a.scheduleId.toString(), a);
    }

    const enrichedItems = items.map((lead) => ({
      ...lead,
      schedule: schedulesMap.get(lead._id.toString()),
      assignment: assignmentsMap.get(
        schedulesMap.get(lead._id.toString())?._id?.toString()
      ),
    }));
